...
public class OMLSpecificationResolverLibraryGenerator extends OMLUtilities {
  ...
  public void generate(final String targetDir) {
    ...
    this.generate(Collections.<EPackage>unmodifiableList(CollectionLiterals.<EPackage>newArrayList(this.c, this.t, this.g, this.b, this.d, this.e)), targetPath.toAbsolutePath().toString());
  }
  ...
  public void generate(final List<EPackage> ePackages, final String targetFolder) {
    try {
      ...
      factoryFile.write(this.generateFactoryFile(ePackages, "gov.nasa.jpl.imce.oml.specification.resolver.impl").getBytes());
      final Function1<EPackage, Iterable<EClass>> _function = (EPackage it) -> {
        return OMLUtilities.FunctionalAPIClasses(it);
      };
      Iterable<EClass> _flatten = Iterables.<EClass>concat(ListExtensions.<EPackage, Iterable<EClass>>map(ePackages, _function));
      for (final EClass eClass : _flatten) {
        {
